...
// Default constructor
Permutation::Permutation() : m(0), n(0), level(0) {}

// Constructor from processing times
Permutation::Permutation(std::vector<std::vector<int>> &p_)
    : level(0)
{
    // Constructor implementation here
    // Set m and n based on the dimensions of p_
    this->m = p_[0].size();
    this->n = p_.size();
...
Permutation::Permutation(int m_, std::vector<JobPtr> &jobs_)
    : m(m_), n(jobs_.size()), level(0), sigma1(m_), free_jobs(jobs_), sigma2(m)
{
    // Constructor implementation here
...
Permutation::Permutation(
    ...
    : m(m_), n(n_), level(level_), sigma1(sigma1_),
      free_jobs(free_jobs_), sigma2(sigma2_)
{
    // Constructor implementation here
    update_params();
}
...
void Permutation::push_job(int &j)
{
...
int Permutation::lower_bound_2m()
{
    ...
    for (int m1 = 0; m1 < this->m; ++m1){
        for (int m2 = m1 + 1; m2 < this->m; ++m2){
            int temp_value = (
                r[m1]
                + two_mach_problem(this->free_jobs, m1, m2)
                + q[m2]
            );
...
// Copy methods
Permutation Permutation::copy() const
{
...
// std::shared_ptr<Permutation> Permutation::shallow_copy()
// {
...
int two_mach_problem(std::vector<JobPtr> &jobs, int &m1, int &m2)
{
    ...
    for (int j = 0; j < jobs.size(); ++j){
        ...
        JobParams jparam = JobParams(
            t1, t2,
            jobs[j]->p->at(m1), jobs[j]->p->at(m2),
            jobs[j]->lat->at(m2)[m1]
        );
        if (t1 <= t2){
            j1.push_back(jparam);
