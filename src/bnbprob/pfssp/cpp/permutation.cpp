...
Permutation::Permutation(const int &m_, const std::vector<JobPtr> &jobs_)
    : m(m_),
      n(jobs_.size()),
      level(0),
      sigma1(m_),
...
Permutation::Permutation(const int &m_, const int &n_, const int &level_,
                         const Sigma &sigma1_,
...
int two_mach_makespan(const std::vector<JobParams> &job_times, const int &m1, const int &m2) {
    // Implementation here
    int time_m1 = 0;
    int time_m2 = 0;
