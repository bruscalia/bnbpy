...
public:
    ...
    MachineGraph(int M,
                 const std::vector<std::vector<int>>& prec,
        ...
        : M(M), prec(prec), succ(succ), topo_order(topo_order), rev_topo_order(rev_topo_order), descendants(descendants) {}
    ...
    MachineGraph(int M,
                 ...
                 const std::vector<int>& topo_order,
                 const std::vector<std::vector<int>>& descendants)
        : M(M), prec(prec), succ(succ), topo_order(topo_order), descendants(descendants) {
    ...
    const std::vector<std::vector<int>>& get_prec_all() const {
        return prec;
    }

    // Getters for successor operations
    const std::vector<std::vector<int>>& get_succ_all() const {
        return succ;
    }

    // Getter for topological order
    const std::vector<int>& get_topo_order() const {
        return topo_order;
    }
