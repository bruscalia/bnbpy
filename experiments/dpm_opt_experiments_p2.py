...
from bnbprob.pafssp.instances import AssemblyFlowShopInstance, dpm_layout_edges
from bnbpy import configure_logfile

configure_logfile('dpm-experiments-p2.log', mode='w')
...
def truncate_instance(
    ...
    m_tot = len(instance.p[0])
    new_m = m_tot - 3
    new_p = [row[3:] for row in instance.p]
    new_edges = dpm_layout_edges(2, new_m - 2)
    return AssemblyFlowShopInstance(new_p, new_edges)
...
if __name__ == '__main__':
    ...
    df_dpm.to_csv('dpm-experiments-p2.csv', index=False)
