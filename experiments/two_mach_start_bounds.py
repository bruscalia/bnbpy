...
from bnbprob.pafssp import CutoffBnB, PermFlowShop
from bnbpy import configure_logfile

configure_logfile('two-mach-start-bounds-experiments.log', mode='w')
...
def run_experiment(  # noqa: PLR0913, PLR0917
    ...
    # Initialization
    problem = PermFlowShop.from_p(p, constructive='quick')
    bnb = CutoffBnB(ub)
    # Compute starting bounds
    lb1 = problem.lower_bound_1m()
    lb5 = problem.lower_bound_2m()
    # Solve
...
if __name__ == '__main__':
    ...
    df_results = pd.DataFrame([asdict(e) for e in experiments])
    df_results.to_csv('two-mach-start-bound-experiments.csv', index=False)
