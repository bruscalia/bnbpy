...
configure_logfile('custom-opt-experiments.log', mode='w')
...
def run_experiment(
    ...
    sol = bnb.solve(problem, maxiter=1_000_000_000, timelimit=timelimit)
...
if __name__ == '__main__':
    ...
    for file in all_files:
        if not file.endswith('.json'):
            continue
        name = file[:-5]
        chain_def = file.split('.')[0].split('-')[-1]
        idx = int(name[7:10])
    ...
    df_custom = pd.DataFrame([asdict(e) for e in experiments])
    df_custom.to_csv('custom-opt-experiments.csv', index=False)
